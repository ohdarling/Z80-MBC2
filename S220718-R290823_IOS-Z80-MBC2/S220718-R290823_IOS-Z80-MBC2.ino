...
// --- SD subdirectory for all IOS files -------------------------------------
// MUST be UPPERCASE 8.3 (PetitFS is built with _USE_LCC = 0, so no toupper is
// applied to the path) and MUST end with '/'.  Set SDDIR to "" to use the SD
// card root as in the original firmware.
#define   SDDIR         "Z80MBC2/"
#define   SDDIRLEN      8                   // = strlen(SDDIR)
// ---------------------------------------------------------------------------
#define   BASICFN       SDDIR "BASIC47.BIN" // "ROM" Basic
#define   FORTHFN       SDDIR "FORTH13.BIN" // "ROM" Forth
#define   CPMFN         SDDIR "CPM22.BIN"  // CP/M 2.2 loader
#define   QPMFN         SDDIR "QPMLDR.BIN" // QP/M 2.71 loader
#define   CPM3FN        SDDIR "CPMLDR.COM" // CP/M 3 CPMLDR.COM loader
#define   UCSDFN        SDDIR "UCSDLDR.BIN" // UCSD Pascal loader
#define   COSFN         SDDIR "COS.BIN"    // Collapse OS loader
#define   FUZIXFN       SDDIR "FUZIX.BIN"  // Fuzix OS loader
#define   AUTOFN        SDDIR "AUTOBOOT.BIN" // Autoboot.bin file
#define   Z80DISK       SDDIR "DSxNyy.DSK" // Generic Z80 disk name (from DS0N00.DSK to DS9N99.DSK)
#define   DS_OSNAME     SDDIR "DSxNAM.DAT" // File with the OS name for Disk Set "x" (from DS0NAM.DAT to DS9NAM.DAT)
...
char          diskName[11 + SDDIRLEN] = Z80DISK;   // String used for virtual disk file name
char          OsName[11 + SDDIRLEN]   = DS_OSNAME; // String used for file holding the OS name
      ...
      //
      {
        ...
        // Execute the requested I/O WRITE Opcode. The 0xFF value is reserved as "No operation".
        {
          ...
          case  0x09:
            ...
            {
              diskName[SDDIRLEN + 2] = diskSet + 48;         // Set the current Disk Set
              diskName[SDDIRLEN + 4] = (ioData / 10) + 48;   // Set the disk number
              diskName[SDDIRLEN + 5] = ioData - ((ioData / 10) * 10) + 48;
...
// The OS name is inside the file defined in DS_OSNAME
{
  ...
  OsName[SDDIRLEN + 2] = currentDiskSet + 48;    // Set the Disk Set
